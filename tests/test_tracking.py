...
from tracking51 import tracking,exception, const
...
def test_empty_tracking_number():
    ...
    with pytest.raises(exception.Tracking51Exception, match=const.ErrMissingTrackingNumber):
...
def test_empty_courier_code():
    ...
    with pytest.raises(exception.Tracking51Exception, match=const.ErrMissingCourierCode):
        tracking.create_tracking(invalid_params)
...
def test_exceed_max_tracking_numbers():
    ...
    with pytest.raises(exception.Tracking51Exception, match=const.ErrMaxTrackingNumbersExceeded):
...
def test_empty_id_string():
    test_params = {'customer_name': 'New name', 'note': 'New tests order note'}
    with pytest.raises(exception.Tracking51Exception, match=const.ErrEmptyId):
