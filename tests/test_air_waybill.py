...
from tracking51 import air_waybill, exception, const
...
def test_empty_awb_number():
    ...
    with pytest.raises(exception.Tracking51Exception, match=const.ErrMissingAwbNumber):
        air_waybill.create_an_air_waybill(invalid_params)

def test_invalid_awb_format():
    invalid_params = {'awb_number': '12345'}
    with pytest.raises(exception.Tracking51Exception, match=const.ErrInvalidAirWaybillFormat):
        air_waybill.create_an_air_waybill(invalid_params)